...
export default function PostsGrid({ posts }: PostsGridProps) {
  return (
    <section
      id="grid"
      className="grid px-4 sm:px-8 gap-8 gap-y-12 sm:gap-x-2 sm:grid-cols-2 xl:grid-cols-4  -mt-6 mb-10"
    >
      {posts.map((post) => (
        <article
        key={post.slug}
        ...
        <div className="flex h-full flex-col overflow-hidden border border-black/10 dark:border-white/10 bg-white dark:bg-neutral-950 shadow-sm hover:shadow-xl transition-shadow duration-300">
            ...
            <Link href={`/blog/${post.slug}`} className="block">
                <h2 className="text-xl font-sham sm:font-sans sm:text-2xl font-bold sm:font-black leading-tight text-black dark:text-white mb-3 transition-colors group-hover:text-sham dark:group-hover:text-voke">
                {post.title}
                </h2>
            </Link>
